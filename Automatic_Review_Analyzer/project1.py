...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    Returns:
        the hinge loss, as a float"""
    z={}
    hinge_loss_sum = 0
    for i in range(len(feature_matrix)):
        z_i = labels[i]*(theta@feature_matrix[i]+theta_0)
        z[f"z {i}"] = z_i
        if z>1:
            hinge_loss = 0;
        else:
    ...
    hinge_loss_average = hinge_loss_sum/len(feature_matrix)

    return hinge_loss_average
...
def perceptron_single_step_update(
    ...
    if z>1:
        ...
        theta_new = theta + label@feature_vector
        theta_0_new = theta_0 + label
...
def average_perceptron(feature_matrix, labels, T):
    ...
    Returns a tuple containing two values:
        the average feature-coefficient parameter `theta` as a numpy
            array (averaged over T iterations through the feature matrix)"""
    theta = np.zeros(len(feature_matrix[0]))
    theta_0 = 0
    sum_of_loss = 0
    samples =200
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    samples =200
    theta = np.zeros(feature_matrix[0])
    theta_0 = 0
    for j in range(T):
        for i in get_order(samples):
            t =1
            eta = 1/np.sqrt(1)
            theta, theta_0 = pegasos_single_step_update(feature_matrix[i],label,L,eta,theta,theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    nsamples =200
    classification = np.zeros(nsamples)
    for i in (get_order(nsamples)):
        hinge_value = theta@feature_matrix[i] +theta_0
        if hinge_value>0:
            classification[i] = 1;
        else:
            classification[i] =-1;
...
def classifier_accuracy(
    ...
    for i in get_order(nsamples):
        ...
        if theta@train_feature_matrix[i]+ theta_0 != train_labels[i]:
            error_train +=1
    training_error = error_train/count_train
    count_val= 0
    error_val= 0

    for i in range(len(val_feature_matrix)):
        count_val +=1
        if theta@val_feature_matrix[i] +theta_0 != val_labels[i]:
            error_val +=1
    validation_error = error_val/count_val
...
def bag_of_words(texts, remove_stopword = False):
    ...
    """
    if remove_stopword:
        stopword = np.loadtxt("stopwords.txt", delimiter ="\n")
    stopword = []
