...
def load_toy_data(path_toy_data):
    ...
    return np.vstack((xs, yx)).T, labels
...
def write_predictions(path_submit_data, preds):
    if PYTHON3:
        f_data = open(path_submit_data, encpding= "latin1")
    else:
    ...
    for pred, datum in zip(preds.astype(int), data):
        assert pred == 1 or pred == -1 , "Invalid predictions: {}.".format(pred)
        datum["sentiment"] = pred
    f_data.close()
    #astype casts one data type to another
...
def tune(train_fn, param_vals, train_feats, train_labels, val_feats, val_labels):
    ...
    val_access = np.ndarray(len(param_vals))
    ...
    for i, val in enumerate(param_vals):
        ...
        val_preds = p1.classify(train_feats, theta, theta_0)
...
def most_explanatory_word(theta, wordlist):
    ...
    return [word for (theta_i, word) in sorted]
